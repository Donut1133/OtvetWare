    ...
    match core.http.request(acc, "/api/auth/user", ReqOpts::get(), stop).await {
        Ok(p) => {
            ...
            if let Some(j) = p.json.as_ref().filter(|_| p.ok) {
                // Ответ обязан быть ПРО НАШ аккаунт.
                //
                // На пути до сайта бывает кэш, и тогда в ответ прилетает чужой
                // профиль. Поверить ему — записать себе чужие id и ник (а id
                // уходит автором в публикацию вопросов) и заодно примерить
                // чужой статус: у соседа бан — покраснеет наш. Сверка бесплатная,
                // id мы и так знаем.
                if let (Some(known), Some(got)) = (acc.user_id, j.get("id").and_then(|v| v.as_i64())) {
                    if known != got {
                        return Validation {
                            error: Some(format!("сайт ответил про чужой аккаунт (id {got}, а не {known})")),
                            ..Default::default()
                        };
                    }
                }
...
/// Сколько ждать перед вторым взглядом на приговор — бан или разлогин.
/// Достаточно, чтобы сайт успел ответить иначе, и мало, чтобы не тормозить круг.
const RECHECK_MS: u64 = 2500;
...
pub async fn validate_account(core: &Core, acc: &Account, stop: &Stop) -> Validation {
    ...
    // На разлогин смотрим дважды.
    //
    // Отказ по авторизации бывает разовым: сайт моргнул, прокси на секунду
    // ответил чужим отказом. Цена ошибки — аккаунт краснеет и вылетает из
    // круга, а нажатая через минуту «Проверить» говорит, что всё в порядке.
    // Ровно на это и жаловались.
    if out.auth_bad && !stop.is_stopped() && !stop.sleep_ms(RECHECK_MS).await {
    ...
    if out.banned && !stop.is_stopped() && !stop.sleep_ms(RECHECK_MS).await {
        let again = auth_probe(core, acc, stop).await;
