...
impl AiClient {
    ...
    pub async fn check(&self, cfg: &AiCfg, stop: &Stop) -> Result<String, String> {
        ...
        // Проверяем ровно тем же телом, что уйдёт в работу: иначе проверка в
        // начале прогона скажет ОК, а на первом же ответе провайдер отвергнет
        // уровень размышлений или чужое поле.
        let body = dress_body(
            cfg,
            json!({
                "model": cfg.model,
...
mod tests {
    ...
    fn own_fields_win_over_everything_else() {
        ...
        // про ошибку скажут форма и проверка в начале прогона.
