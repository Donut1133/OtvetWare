// stealth.js — main-world инжект отпечатка, дословно перенесённый из
// JS-версии бота (там он и отлаживался бисекцией по живому антифроду).
//
// Правится ТОЛЬКО вместе с оригиналом: персоны у двух версий общие, и если
// шум ляжет иначе, один и тот же аккаунт станет отдавать разный canvas-хэш —
// то есть ровно тот скачок, ради предотвращения которого всё это написано.
//
