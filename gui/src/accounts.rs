                ...
                let acc = match (existing, core.accounts.get(&name)) {
                    (true, Some(a)) => {
                        core.accounts.mutate(&name, |x| {
                            ...
                            x.banned = None;
                            // И личность учим заново: под тем же именем могли
                            // войти в другой аккаунт, а неверный id в базе
                            // теперь заставляет проверку отвергать ответы сайта
                            // как «про чужой аккаунт». Проверка ниже подставит
                            // настоящие id и ник.
                            x.user_id = None;
                            x.username = None;
                        });
