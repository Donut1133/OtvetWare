...
impl CommonForm {
    ...
    ///
    /// `shared_work` — работа общая на все аккаунты (сейчас это диапазон
    /// номеров). Лимиты там значат другое: не «сколько сделает каждый», а
    /// «сколько отхватит из общей кучи», и об этом надо сказать вслух.
    pub fn ui(&mut self, ui: &mut Ui, selected: usize, feed_mode: bool, shared_work: bool) {
        block(ui, "Аккаунты");
        ui.horizontal(|ui| {
        ...
        extra(ui, title, "common_extra", |ui| {
            if feed_mode {
                ui.horizontal(|ui| {
                    ui.label("Лимит на аккаунт: за прогон");
                ...
                hint(
                    ui,
                    "0 = без ограничения. Первый считается за весь прогон вместе с повторными кругами, второй — за один круг. Работает самый строгий из них и лимита в самой панели режима.",
                );
                if shared_work {
                    hint(
                        ui,
                        "В диапазоне номера общие на всех, так что «за круг» — это сколько номеров аккаунт разберёт и уступит остальным. Следующий круг продолжит с того места, где остановился прошлый, а не начнёт диапазон заново.",
                    );
                }
                ui.horizontal(|ui| {
...
pub fn uniq_block(ui: &mut Ui, mode: &mut UniqMode, latin: &mut bool) {
    ...
    if *mode == UniqMode::Hard {
        hint(
            ui,
            "«Сильная» вдобавок дописывает в конец три-шесть случайных цифр через пробел. Не решёткой: за «#293508» модерация цепляется, за «4821» — нет.",
        );
    }
}
...
"
            ...
            (
                ...
                Box::new({
                    ...
                    // С общей работой подсказок больше — меряем худший случай.
                    move |ui: &mut Ui| f.ui(ui, 3, true, true)
                }),
            ),
        ];
